...
class PasswordResetFormKeyView(FormView):
    ...
    def dispatch(self, request, uidb36, key, **kwargs):
        ...
        if not self.token_generator.check_token(self.request.user, key):
            ...
            return super(PasswordResetFormKeyView, self).dispatch(request, uidb36, key, **kwargs)

    def get_form_kwargs(self):
        kwargs = super(PasswordResetFormKeyView, self).get_form_kwargs()
        kwargs["user"] = self.request.user
    ...
    def form_valid(self, form):
        ...
        signals.password_reset.send(sender=self.request.user.__class__,
                request=self.request, user=self.request.user)
        return super(PasswordResetFormKeyView, self).form_valid(form)
...
password_reset_from_key = PasswordResetFormKeyView.as_view()


class PasswordResetFormKeyDoneView(TemplateView):
    template_name = "account/password_reset_form_key_done.html"

password_reset_form_key_done = PasswordResetFormKeyDoneView.as_view()
