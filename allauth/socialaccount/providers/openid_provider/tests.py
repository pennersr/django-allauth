...
class OpenIDTests(TestCase):
    ...
    def test_login(self):
        ...
        with patch(
            "allauth.socialaccount.providers.openid_provider.views._openid_consumer"
        ) as consumer_mock:
            client = Mock()
            complete = Mock()
            ...
            with patch(
                "allauth.socialaccount.providers.openid_provider.utils.SRegResponse"
            ) as sr_mock:
                with patch(
                    "allauth.socialaccount.providers.openid_provider.utils.FetchResponse"
                ) as fr_mock:
                    sreg_mock = Mock()
                    ax_mock = Mock()
    ...
    def test_login_with_extra_attributes(self):
        with patch("allauth.socialaccount.providers.openid_provider.views.QUERY_EMAIL", True):
            resp = self.client.post(
                reverse(views.login), dict(openid="https://steamcommunity.com/openid")
            )
        assert "steamcommunity.com/openid/login" in resp["location"]
