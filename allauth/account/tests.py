...
class AccountTests(TestCase):
    ...
    def test_username_containing_at(self):
        ...
        EmailAddress.objects.create(
            user=user, email="raymond.penners@example.com", primary=True, verified=True
        )
        resp = self.client.post(
            reverse("account_login"), {"login": "@raymond.penners", "password": "psst"}
        )
    ...
    @override_settings(
        ACCOUNT_USERNAME_REQUIRED=True, ACCOUNT_SIGNUP_PASSWORD_ENTER_TWICE=True
    )
    def test_signup_password_twice_form_error(self):
        ...
        self.assertFormError(
            resp, "form", "password2", "You must type the same password each time."
        )
    ...
    def test_password_set_redirect(self):
        ...
        self.assertRedirects(
            resp, reverse("account_change_password"), fetch_redirect_response=False
        )
    ...
    def test_set_password_not_allowed(self):
        ...
        resp = self.client.post(
            reverse("account_set_password"), data={"password1": pwd, "password2": pwd}
        )
    ...
    def test_password_change_redirect(self):
        ...
        self.assertRedirects(
            resp, reverse("account_set_password"), fetch_redirect_response=False
        )
    ...
    def _request_new_password(self):
        ...
        user.save()
        self.client.post(
            reverse("account_reset_password"), data={"email": "john@example.org"}
        )
    ...
    def test_email_verification_mandatory(self):
        ...
        for attempt in [1, 2]:
            ...
            self.assertTemplateUsed(
                resp, "account/verification_sent." + app_settings.TEMPLATE_EXTENSION
            )
        ...
        resp = c.post(
            reverse("account_login"), {"login": "johndoe", "password": "johndoe"}
        )
        self.assertRedirects(
            resp, settings.LOGIN_REDIRECT_URL, fetch_redirect_response=False
        )
    ...
    def test_login_failed_attempts_exceeded(self):
        ...
        for i in range(5):
            ...
            resp = self.client.post(
                reverse("account_login"),
                {"login": "john", "password": ("doe" if is_valid_attempt else "wrong")},
            )
            self.assertFormError(
                resp,
                "form",
    ...
    def test_ajax_login_fail(self):
        resp = self.client.post(
            reverse("account_login"), {}, HTTP_X_REQUESTED_WITH="XMLHttpRequest"
        )
    ...
    @override_settings(
        ACCOUNT_EMAIL_CONFIRMATION_HMAC=True, ACCOUNT_EMAIL_CONFIRMATION_EXPIRE_DAYS=0
    )
...
class EmailFormTests(TestCase):
    def setUp(self):
        ...
        self.email_address2 = EmailAddress.objects.create(
            user=self.user, email="john2@example.org", verified=False, primary=False
        )
    ...
    def test_add(self):
        ...
        EmailAddress.objects.get(
            email="john3@example.org", user=self.user, verified=False, primary=False
        )
...
class BaseSignupFormTests(TestCase):
    ...
    def test_username_in_blacklist(self):
        data = {"username": "username", "email": "user@example.com"}
    ...
    def test_username_not_in_blacklist(self):
        data = {"username": "theusername", "email": "user@example.com"}
        form = BaseSignupForm(data, email_required=True)
...
class CustomSignupFormTests(TestCase):
    @override_settings(
        ACCOUNT_SIGNUP_EMAIL_ENTER_TWICE=True, ACCOUNT_SIGNUP_PASSWORD_ENTER_TWICE=True
    )
...
class ConfirmationViewTests(TestCase):
    ...
    @override_settings(
        ACCOUNT_EMAIL_CONFIRMATION_HMAC=True, ACCOUNT_LOGIN_ON_EMAIL_CONFIRMATION=True
    )
