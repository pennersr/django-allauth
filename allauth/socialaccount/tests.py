...
class OAuth2TestsMixin:
    ...
    def test_account_tokens(self, multiple_login=False):
        ...
        if multiple_login:
            self.login(
                self.get_mocked_response(), with_refresh_token=False, process="connect"
            )
        # get account
...
class SocialAccountTests(TestCase):
    ...
    def test_populate_username_in_blacklist(self):
        ...
        setattr(user, account_settings.USER_MODEL_EMAIL_FIELD, "username@example.com")
    ...
    def test_verified_email_change_at_signup(self):
        ...
        resp = self.client.post(
            reverse("socialaccount_signup"), data={"email": "unverified@example.org"}
        )
        self.assertRedirects(resp, "/accounts/profile/", fetch_redirect_response=False)
        ...
        self.assertTrue(
            EmailAddress.objects.filter(
                user=user, email="unverified@example.org", verified=False, primary=False
            ).exists()
        )
    ...
    def test_unverified_email_change_at_signup(self):
        ...
        self.assertTrue(
            EmailAddress.objects.filter(
                user=user, email="unverified@example.org", verified=False, primary=True
            ).exists()
        )
