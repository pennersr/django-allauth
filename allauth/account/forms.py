...
from ..utils import build_absolute_uri, get_username_max_length, set_form_field_order
...
class PasswordField(forms.CharField):
    def __init__(self, *args, **kwargs):
        ...
        kwargs["widget"] = forms.PasswordInput(
            render_value=render_value, attrs={"placeholder": kwargs.get("label")}
        )
...
class LoginForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if app_settings.AUTHENTICATION_METHOD == AuthenticationMethod.EMAIL:
            ...
            login_widget = forms.TextInput(
                attrs={"placeholder": _("Username or e-mail"), "autofocus": "autofocus"}
            )
            login_field = forms.CharField(
...
class AddEmailForm(UserForm):
    ...
    email = forms.EmailField(
        ...
        required=True,
        widget=forms.TextInput(
            attrs={"type": "email", "size": "30", "placeholder": _("E-mail address")}
        ),
    )
...
class UserTokenForm(forms.Form):
    ...
    error_messages = {"token_invalid": _("The password reset token was invalid.")}
